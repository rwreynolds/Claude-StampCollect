...
class TestEnhancedStampGUICore(unittest.TestCase):
    ...
    def test_clear_form_functionality(self):
        ...
        for field in field_names:
            element = MagicMock()
            element.update = MagicMock()  # Explicitly create update mock
            form_elements[field] = element
        
        # Mock condition and gum dropdowns
        for field in ["condition_grade", "gum_condition"]:
            element = MagicMock()
            element.update = MagicMock()
            form_elements[field] = element
        ...
        for checkbox in checkbox_names:
            element = MagicMock()
            element.update = MagicMock()
            form_elements[checkbox] = element
        
        def mock_find_element(key):
            return form_elements.get(key, MagicMock())
        ...
        # Verify updates were called with correct values
        for field in field_names:
